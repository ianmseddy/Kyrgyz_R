...
setwd("D:/MODIS/Chapter 2/")

ndviR2 <- raster("Test/MayRain/mrNDVI_tot_r2.tif")
ndvisw <- raster("Test/MayRain/mrNDVI_sw.tif")
ndvibg <- raster("Test/MayRain/mrNDVI_bg.tif")
ndvibp <- raster("Test/MayRain/mrNDVI_bp.tif")
ndvi_sg <- raster("Test/MayRain/Restrend_mkSg.tif")
ndvi_pval <- raster("Test/MayRain/Restrend_mkpval.tif")
...
writeRaster(NDVI_trendPos, filename = "Test/MayRain/PostR_Analyses/ndvi_trend_pos05.tif", overwrite = TRUE)
writeRaster(NDVI_trendNeg, filename = "Test/MayRain/PostR_Analyses/ndvi_trend_neg05.tif", overwrite = TRUE)


m1 <- c(0,1,1)
m11 <- matrix(m1, ncol=3,byrow=TRUE)
ndvipot <- reclassify(ndvipot, m11)
writeRaster(ndvipot, filename = "Test/MayRain/PostR_Analyses/ndvipot.tif", overwrite = TRUE)
